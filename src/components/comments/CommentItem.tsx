// src/components/comments/CommentItem.tsx (전체 코드 다시 확인용)
...
const CommentItem: React.FC<CommentItemProps> = ({ comment, onDelete, onReply, level }) => {
  ...
  return (
    <ListItem
      ...
      divider
      sx={{ pl: level * 3, bgcolor: level > 0 ? `rgba(0, 0, 0, ${level * 0.03})` : 'inherit' }}
    >
      ...
      />
      {onDelete && (
          <IconButton edge="end" aria-label="delete" onClick={handleDeleteClick} size="small" sx={{ mt: 1 }}>
